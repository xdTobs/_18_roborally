...
public class CardFieldView extends GridPane implements ViewObserver {
    ...
    private final CommandCardField commandCardField;

    private final Label label;


    public CardFieldView(Board board, @NotNull CommandCardField commandCardField) {
        this.board = board;
    ...
    private String cardFieldRepresentation(CommandCardField cardField) {
        if (cardField.player != null) {
            ...
            for (int i = 0; i < Player.NO_REGISTER_CARDS; i++) {
                CommandCardField other = cardField.player.getRegisterCardField(i);
                if (other == cardField) {
                    return "P," + i;
                }
            }
            ...
            for (int i = 0; i < Player.NO_PLAYABLE_CARDS; i++) {
                CommandCardField other = cardField.player.getPlayableCardField(i);
                if (other == cardField) {
                    return "C," + i;
                }
            }
        ...
        if (rep != null && commandCardField.player != null) {
            ...
            if (strings.length == 2) {
                ...
                if ("P".equals(strings[0])) {
                    if (i < Player.NO_REGISTER_CARDS) {
                        return commandCardField.player.getRegisterCardField(i);
                    }
                } else if ("C".equals(strings[0])) {
                    if (i < Player.NO_PLAYABLE_CARDS) {
                        return commandCardField.player.getPlayableCardField(i);
                    }
                }
            }
        }
    ...
    private class OnDragDetectedHandler implements EventHandler<MouseEvent> {
        ...
        public void handle(MouseEvent event) {
            ...
            if (t instanceof CardFieldView source) {
                ...
                if (cardField.getCard() != null && cardField.player != null && board != null && board.getPhase().equals(Phase.PROGRAMMING)) {
    ...
    private class OnDragOverHandler implements EventHandler<DragEvent> {
        ...
        public void handle(DragEvent event) {
            ...
            if (t instanceof CardFieldView target) {
                ...
                if ((cardField.getCard() == null || event.getGestureSource() == target) && cardField.player != null && board != null) {
    ...
    private class OnDragEnteredHandler implements EventHandler<DragEvent> {
        ...
        public void handle(DragEvent event) {
            ...
            if (t instanceof CardFieldView target) {
                CommandCardField cardField = target.commandCardField;
                if (cardField.getCard() == null && cardField.player != null && board != null) {
